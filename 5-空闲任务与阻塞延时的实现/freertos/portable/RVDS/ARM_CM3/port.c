...
BaseType_t xPortStartScheduler( void )
{
    ...
    /* 配置PendSV 和 SysTick 的中断优先级为最低 */
	  /* 因为 PendSV 和 SysTick 都会涉及到系统的调度，
			系统的调度优先级要比系统的其他硬件优先级要低，
			防止系统抢占了其他硬件的外部中断 */
...
__asm void prvStartFirstTask( void )
{
	...
	ldr r0, =0xE000ED08      //将 0xE000ED08 加载到 r0 中
	ldr r0, [r0]             //将 0xE000ED08 指向的地址加载到r0中，此时r0的值等于SCB_VTOR的值0x00000000
	ldr r0, [r0]						 //将地址指向的内容加载到r0

	/* 设置主堆栈指针msp的值为r0 */
	msr msp, r0			//主栈指针的栈顶指针
...
__asm void vPortSVCHandler( void )
{
  ...
  // #0xd ：按位与上oxod     使得硬件在退出时使用进程堆栈指针PSP完成出栈操作并返回后进入线程模式、返回Thumb状态 */
...
{
	extern pxCurrentTCB;        //声明一个外部变量，指向当前任务控制块的指针
	extern vTaskSwitchContext;  //声明一个外部函数，用于选择
